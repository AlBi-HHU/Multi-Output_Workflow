...
def L(y, K, alpha, b, eps):
    ...
    Lu = u**2 - 2*u*eps + eps**2
    Lu[u < eps] = 0
    return np.sum(Lu)

def Lp(y, K, alpha, b, eps, C):
    return 0.5*np.trace(alpha.T@K@alpha) + C/2*L(y, K, alpha, b, eps) # C/2 for same setup as LS-SVR

def IRWLS(K, y, C, eps2, tol):
    n, p = y.shape
    alpha = np.zeros((n, p))
    b = np.zeros(p)
    ...
    while eta > tol:
        ...
        a = 2*C*(u - eps2)/u # warning with division by u=0, but as u < eps2, entry will be set to 0 in next line
        a[u < eps2] = 0
        D = np.diag(a)
...
alpha, b = IRWLS(K, y_train, C, eps2, tol)
